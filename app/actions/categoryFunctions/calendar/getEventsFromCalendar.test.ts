...
describe('getEventsFromCalendar', () => {
    ...
    it('returns error message when user not signed in', async () => {
        ...
        const result = await getEventsFromCalendar(
    ...
    it('asks for calendar name when it is missing', async () => {
        const result = await getEventsFromCalendar(
            '', 
            mockCurrentDate, 
            mockCurrentTime
    ...
    it('gets events from requested calendar', async () => {
        ...
        expect(result.outputText).toMatch(/your future events|you have no future events/i);
    })
