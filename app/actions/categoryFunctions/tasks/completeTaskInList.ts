...
export default async function completeTaskInList(details: TaskDetails) {
    const session = await auth();
    if (!session)
        return {outputText: 'You need to be signed in to access your tasks.'}
    ...
    if (!requestedTaskName)
        return {outputText: `Sorry, I can't complete the task because you didn't provide a task name.`}
    ...
    try {
        ...
        return {outputText: `Sorry, I couldn't mark your task as complete.`}
    }
    ...
    const listName = taskListData.items.find((list: TaskList) => list.id === taskListId)?.title;
    const taskName = listTasks.items.find((task: TaskItem) => task.id === taskId)?.title;

    return {outputText: `I marked "${taskName}" as complete in your ${listName} list.`}
}
