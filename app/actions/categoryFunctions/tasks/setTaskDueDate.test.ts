...
describe('setTaskDueDate', () => {
    ...
    it('returns error message when user not signed in', async () => {
        ...
        const result = await setTaskDueDate(mockTaskDetails);

        expect(result.outputText).toMatch(/sign.*in/i);
    })

    it('sets due date of given task', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith(expect.any(String),
            expect.objectContaining({
                method: 'PATCH',
                body: expect.stringMatching(/^(?=.*2024)(?=.*11)(?=.*30).*$/)
            })
        )
        ...
        expect(result.outputText).toMatch(/set/i);
    })
